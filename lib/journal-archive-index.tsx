...
export type JournalArchiveIndexProps = {
  searchParams?: Promise<{ collection?: string }>;
  /** SEO canonical when this URL is a legacy alias (e.g. /library, /series/foo). */
  canonicalPath?: string;
};

export async function buildJournalArchiveMetadata({
  searchParams,
  canonicalPath,
}: JournalArchiveIndexProps): Promise<Metadata> {
  ...
  const description = (series.tagline || series.description)
    .replace(/\s+/g, " ")
    .trim();
  const defaultCanonical = isPrimary
    ? JOURNAL_INDEX
  ...
  return {
    ...
    description,
    alternates: {
      canonical: canonicalPath ?? defaultCanonical,
    },
  };
}

export async function JournalArchiveIndex({
  searchParams,
}: JournalArchiveIndexProps) {
  const sp = (await searchParams) ?? {};
  const activeSlug = resolveActiveCollectionSlug(sp.collection);
  const series = getSeriesBySlug(activeSlug);
  if (!series) notFound();
